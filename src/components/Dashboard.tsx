  ...
  return (
    <div className="min-h-screen py-2 sm:py-6">
      <div className="container mx-auto px-2 sm:px-4 max-w-4xl">
        ...
        {/* Табы */}
        <div className="flex space-x-2 mb-4 sm:mb-6 overflow-x-auto pb-2 sm:pb-0 px-2 sm:px-0">
          <button 
            className={`flex-shrink-0 px-4 sm:px-6 py-2.5 sm:py-3 rounded-xl text-sm sm:text-base font-medium transition-all duration-300 transform hover:scale-105 whitespace-nowrap ${
              selectedTab === 'all' 
        ...
        {/* Загрузка */}
        {isLoading && (
          <div className="backdrop-blur-lg bg-white/5 rounded-xl sm:rounded-2xl border border-white/10 p-4 sm:p-8 text-center animate-fade-in">
            <div className="inline-block animate-bounce">
              <svg className="animate-spin h-16 sm:h-24 w-16 sm:w-24 text-blue-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="8"></circle>
                <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
              </svg>
            </div>
            <p className="text-gray-300 mt-3 sm:mt-4 text-sm sm:text-base animate-pulse">Загрузка NFT...</p>
          </div>
        )}
        ...
        {!isLoading && nfts.length === 0 && (
          <div className="backdrop-blur-lg bg-white/5 rounded-xl sm:rounded-2xl border border-white/10 p-4 sm:p-8 text-center animate-fade-in">
            <svg className="mx-auto h-12 sm:h-20 w-12 sm:w-20 text-gray-600 animate-pulse" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {/* Сетка NFT */}
        {!isLoading && nfts.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-3 sm:gap-4 px-2 sm:px-0">
